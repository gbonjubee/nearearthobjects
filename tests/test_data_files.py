...
class TestDataFiles(unittest.TestCase):
    ...
    def test_data_files_are_not_empty(self):
        try:
            self.assertTrue(self.neo_file.stat().st_size > 0, "Empty NEO "
                                                              "file.")
            self.assertTrue(self.cad_file.stat().st_size > 0, "Empty CAD "
                                                              "file.")
        except OSError:
    ...
    def test_data_files_are_well_formatted(self):
        ...
        try:
            ...
            raise self.failureException(f"{self.neo_file!r} is not a "
                                        f"well-formated CSV.") from err
        ...
        try:
            ...
            raise self.failureException(f"{self.cad_file!r} is not a valid "
                                        f"JSON document.") from err
