...
def write_to_csv(results, filename):
    ...
    :param results: An iterable of `CloseApproach` objects. :param filename:
...
def write_to_json(results, filename):
    ...
    """
    with open(filename, 'w') as outfile:
        json_formatted = list(map(lambda result: result.to_json(), results))
        json.dump(json_formatted, outfile, indent=2)
