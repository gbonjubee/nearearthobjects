...
The `query` subcommand searches for close approaches that match given criteria:
    ...
    $ python3 main.py query --date 1969-07-29 $ python3 main.py query
    --start-date 2020-01-01 --end-date 2020-01-31 --max-distance 0.025 $
    python3 main.py query --start-date 2050-01-01 --min-distance 0.2
    --min-velocity 50 $ python3 main.py query --date 2020-03-14
    --max-velocity 25 --min-diameter 0.5 --hazardous $ python3 main.py query
    --start-date 2000-01-01 --max-diameter 0.1 --not-hazardous $ python3
    main.py query --hazardous --max-distance 0.05 --min-velocity 30
...
def make_parser():
    ...
    inspect_id.add_argument('-n', '--name',
                            help="The IAU name of the NEO to inspect (e.g. "
                                 "'Halley').")
    ...
    filters.add_argument('--min-distance', dest='distance_min', type=float,
                         help="In astronomical units. Only return close "
                              "approaches that "
                              "pass as far or farther away from Earth as the "
                              "given distance.")
    ...
    filters.add_argument('--hazardous', dest='hazardous', default=None,
                         ...
                         help="If specified only return close approaches of "
                              "NEOs that are potentially hazardous.")
...
def query(database, args):
    ...
    as parsed by the top-level parser.
    """
...
class NEOShell(cmd.Cmd):
    ...
    def parse_arg_with(cls, arg, parser):
        ...
        :param arg: The additional text supplied after the command. :param
        parser: An `argparse.ArgumentParser` to parse the arguments.
    ...
    def precmd(self, line):
        ...
        changed = [f for f in PROJECT_ROOT.glob('*.py') if f.stat().st_mtime >
                   _START]
        if changed:
            value = ', '.join(str(f.relative_to(PROJECT_ROOT)) for f in
                              changed)
            print("The following file(s) have been modified since this "
                  "interactive session began: "  f"{value}",
                  file=sys.stderr)
