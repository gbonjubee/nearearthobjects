...
subclasses of `AttributeFilter` - a 1-argument callable (on a
`CloseApproach`) constructed from a comparator (from the `operator` module),
a reference value, and a class method `get` that subclasses can override to
fetch an attribute of interest from the supplied `CloseApproach`.
...
class AttributeFilter:
    ...
    def __repr__(self):
        ...
        :return: `repr(self)`. The machine-readable string that captures the
...
class DateFilter(AttributeFilter):
    """Query close approaches with date attributes."""

    @classmethod
    def get(cls, approach):
        """
        Get close approaches with date filter.

        :param approach:  A `CloseApproach` on which to evaluate this filter.
        :return: the value of the close approaches after applying date
        filter
        """
...
class DistanceFilter(AttributeFilter):
    """Query close approaches with distance attributes."""

    @classmethod
    def get(cls, approach):
        """
        Get close approaches with distance filter.

        :param approach:  A `CloseApproach` on which to evaluate this filter.
        :return: the value of the close approaches after applying distance
        filter
        """
...
class VelocityFilter(AttributeFilter):
    """Query close approaches with velocity attributes."""

    @classmethod
    def get(cls, approach):
        """
        Get close approaches with velocity filter.

        :param approach:  A `CloseApproach` on which to evaluate this filter.
        :return: the value of the close approaches after applying velocity
        filter
        """
...
class DiameterFilter(AttributeFilter):
    """Query close approaches with diameter attributes."""

    @classmethod
    def get(cls, approach):
        """
        Get close approaches with diameter filter.

        :param approach:  A `CloseApproach` on which to evaluate this filter.
        :return: the value of the close approaches after applying diameter
        filter
        """
...
class HazardousFilter(AttributeFilter):
    """Query close approaches with hazardous attributes."""

    @classmethod
    def get(cls, approach):
        """
        Get close approaches with hazardous filter.

        :param approach: A `CloseApproach` on which to evaluate this filter.
        :return: the value of the close approaches after applying hazardous
        filter
        """
...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    `None` if not specified at the command line (in particular, this means
    that the `--not-hazardous` flag results in `hazardous=False`, not to be
